...
There are many brain atleses avalibe in `ggseg`, but we will just use the Desikan-Killany cortical atlas ('dk') for now, and colour by regions .
```{r}
...
```{r}
regions <- data.frame(cbind(region=c("superior frontal","insula"),
                            value=c(2.4,5.5)),hemi=c("right","right"))

ggseg(regions,mapping=aes(fill=as.numeric(value)), hemi= c("right"), colour="white",size=.3) 

```

Try completing the code below to select some other regions and assigne values to them:

```{r}
#HINT: you can view the names of the ROIs in this atlas by calling base_atlas$region
other_regions <- data.frame(cbind(region=c(""),
                            value=c()),hemi=c(""))

ggseg(other_regions,mapping=aes(fill=as.numeric(value)), hemi= c("right", "left"), colour="white",size=.3) 
