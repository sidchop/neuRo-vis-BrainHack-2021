...
Lets just get a better understading of the input matrix we are working with by visulaising just the matrix:
```{r}
library("lattice")
...
brainconn(atlas ="schaefer300_n7", 
          conmat=x, node.size = 3, 
          view="ortho")
```

Try chaging default view to either "top", "bottom", "left" or "right". Also try changing some of the the other node, edge and background properties. `all.nodes` is an option which displays all nodes of the atlas, rather than just the ones that have connected edges.  
```{r}
brainconn(atlas ="schaefer300_n7", 
          conmat=x,
          view="left",
...
```
If the input is a directed matrix (i.e. non-symmetrical), the edges will be displayed as directed arrows, with the `edge.width` and `edge.color` serving the same putpose. Try plotting the example unweighted_directed matrix:
