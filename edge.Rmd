...
We will now move onto visualising edge-level data. somtimes the result of our neuroimaging analyses result in egge-level values, in the form of a adjcency or connectivity matrix. The primary input is a n by n square-matrix, where n is the number of brain regions (aka nodes), and each elemet of a matrix related to a connectivity value (aka an edge). We will primarly be using the `brainconn` package which allows us to plot brain networks in 2d and interactive 3d. 
...
```
Since the example matric has 300 rois, which relate to the Schaefer 300 atlas, we will plot it using the same atlas. `brainconn` comes with many atlas, and it is simple to add your own [add link to atlas creation docs].
