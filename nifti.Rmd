...
We can adjust the brightness of the image by changing the upper and lower limits of the colours shown using `zlim`. 
```{r}
image(img, z = 35, plot.type = "single", zlim = c(0, 1500))
```

We can also change the plane, and view multiple specific slices:

```{r}
image(img, z = c(40,45, 50,55), plot.type = "single", plane = "sagittal", zlim = c(0, 1500))
```

Or we can just view all slices, which can be useful for checking quality of images:

```{r}
image(img, zlim = c(0, 1500))
```
...
#lets try a gorup level

```{r}

template <- readnii("data/MNI152_T1_1mm_brain.nii.gz")
effect <- readnii("data/frdaltcont_Cohens_d_1mm.nii.gz")
image(template, z = 60, plot.type = "single", plane = "sagittal")
image(effect , z = 60, plot.type = "single", plane = "sagittal")


effect[effect<0.4] <- NA

print(template)
print(effect)
overlay(template, y = effect, z = c(seq(1,100, by = 10)) , plot.type = "single", col.y = "red", bg = "white",NA.x = T)

ortho2(x = template, y = effect, crosshairs = F, bg = "white", NA.x = T, add.orient = T, col.y  = hotmetal(), ycolorbar = T)
```
